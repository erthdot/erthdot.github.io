...
The **Quotation Generator** (new — see below) also needs a rule, but it's
admin-only, no public read needed:

```
match /quotations/{id} {
  allow read, write: if request.auth != null; // dashboard-only access
}
```
...
## New: Quotation Generator (admin dashboard)

A full quotation builder is now on the dashboard sidebar under **Tools →
Quotations**:

- Pick real packages straight from your live price list (logo, brand kits,
  social media, à la carte) — prices auto-fill, or add fully custom line
  items with your own description/qty/price.
- Automatic subtotal, discount (% or flat K), total, deposit due, and
  balance — matches the 50%-deposit pattern already used across the site.
- **Copy as Text** — plain-text quote ready to paste anywhere.
- **Send via WhatsApp** — opens WhatsApp pre-filled with the formatted
  quote, straight to the client's number if you entered one.
- **Download / Print PDF** — opens a clean, printable quote in a new tab;
  "Print" → "Save as PDF" from there (works on every device, no extra
  library needed).
- **Save Quotation** — stores it in Firestore (`quotations` collection) so
  you have a running history, reloadable back into the form to edit/resend
  or delete. See the Firestore rule below — this collection is admin-only,
  not public.
